...
public class MainActivityObject extends AppCompatActivity implements
        FragmentOnAttachListener,
        BaseArFragment.OnSessionConfigurationListener {

    private final List<CompletableFuture<Void>> futures = new ArrayList<>();
    private ArFragment arFragment;
    ViewRenderable textDialog;
        ...
        if (savedInstanceState == null) {
            if (Sceneform.isSupported(this)) {
                getSupportFragmentManager().beginTransaction()
                        .add(R.id.arFragment2, ArFragment.class, null)
                        .commit();
            }
        }
    ...
    public void onAttachFragment(@NonNull FragmentManager fragmentManager, @NonNull Fragment fragment) {
        if (fragment.getId() == R.id.arFragment2) {
    ...
    private void loadMatrixModel() {
        WeakReference<MainActivityObject> weakActivity = new WeakReference<>(this);
        ViewRenderable.builder()
                .setView(this, R.layout.text_object_display1)
                .build().thenAccept(renderable -> textDialog = renderable);
    ...
    public void onAugmentedImageTrackingUpdate(AugmentedImage augmentedImage) {
        ...
        if (augmentedImage.getTrackingState() == TrackingState.TRACKING
            ...
            displayText.select();
            title(anchorNodeText, displayText, "webbitto");
            /////end textviewbox========================================
    ...
    private void title(AnchorNode anchorNodeText, TransformableNode displayText, String text) {
        TransformableNode textAtModel = new TransformableNode(arFragment.getTransformationSystem());
        textAtModel.setRenderable(textDialog);
        textAtModel.setLocalPosition(new Vector3(1f, displayText.getLocalPosition().y + 1f, 0));
        textAtModel.setParent(anchorNodeText);
        textAtModel.select();

        TextView arFurDetailsTV = (TextView) textDialog.getView();
        arFurDetailsTV.setText(text);
